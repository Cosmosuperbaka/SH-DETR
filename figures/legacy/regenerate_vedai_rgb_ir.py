#!/usr/bin/env python3
"""Regenerate the VEDAI 2x4 qualitative comparison with RGB+IR stacked panels.

Changes vs the earlier version (addresses reviewer comments):
  ...
  * an arrow on the CMFC-DETR panel highlights a tail-class (camping car)
    target for which CMFC-DETR outputs markedly higher confidence than the
...
IR_IMAGE_PATH = Path("/home/denglingjun/vedai_data/datasets/VEDAI/Vehicules1024/00000025_ir.png")
GT_PATH = Path("/home/denglingjun/vedai_data/datasets/VEDAI/annotations/vedai_fold01_test_class8.json")
OUTPUT_PATH = ROOT / "CMFC_DETR_unpacked/figures/qualitative_vedai_rgb_ir_2x4.png"
...
COLUMN_GAP = 14
ROW_GAP = 30
ARROW_NOTE_BAND = 46       # bottom band carrying the arrow explanation
BOX_WIDTH = 4
...
def main() -> None:
    ...
    for index, name in enumerate(METHOD_ORDER):
        ...
        # Baseline-anchored title: keeps glyph height uniform even for names
        # with descenders (see regenerate_m3fd_rgb_ir.py for rationale).
        title_x = panel_left + MODALITY_SIZE // 2
    ...
    # arrow on the CMFC-DETR panel: tail-class camping car at (787,76)-(827,134)
    cmfc_panel_left = CANVAS_MARGIN + 3 * (MODALITY_SIZE + COLUMN_GAP)
    cmfc_image_top = CANVAS_MARGIN + (panel_height + ROW_GAP)  # second row
    target = (787 + 827) / 2.0, (76 + 134) / 2.0
    end = (
        cmfc_panel_left + target[0] * scale_x,
        cmfc_image_top + TITLE_BAND + target[1] * scale_y,
    )
    note = "Arrow: a tail-class (camping car) target that CMFC-DETR localizes with markedly higher confidence."
    ...
    print(f"canvas={canvas_width}x{canvas_height} image={IMAGE_KEY}")
