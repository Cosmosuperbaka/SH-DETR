...
stacked panels and a genuine CMFC-only high-confidence detection.
...
* detection/matching threshold is 0.70; the tractor target at (377,734,425,778)
  is the only target for which CMFC-DETR outputs a high-confidence detection
  (0.803 vs 0.69/0.67/0.61/0.41/0.39 for the next best), so it is drawn in
  yellow as the CMFC-only detection;
...
OUTPUT_PATH = ROOT / "CMFC_DETR_unpacked/figures/qualitative_vedai_rgb_ir_1033_2x4.png"
...
def main() -> None:
    ...
    # arrow on the CMFC-DETR panel: the CMFC-only high-confidence tractor at (377,734)-(425,778)
    ...
    note = "Arrow: the tractor target detected with high confidence only by CMFC-DETR."

    # legend: red = detection, yellow = CMFC-only true positive
